"""
Use the GPT API to perturb numerical values in a math dataset and recalculate the solution
and answer. Supports processing both the paraphrased and original versions of the data.
...
# Process the original split (fields: original_problem, original_solution, original_answer)
...
python clean_math_dataset_multi_models.py \
    ...
    --output-dir <path/to/output_dir> \
    --model o3 \
    --verify-model-gemini gemini-2.5-flash-lite \
...
Reasoning: [Brief explanation of your verification]"""
    ...
    def paraphrase_dataset(
        ...
        Args:
            ...
            mode: rewriting mode — "paraphrased", "original", or "both"
        ...
        try:
            ...
            if mode == "clean" or dataset_path.endswith('.jsonl'):
        ...
        # Ensure every sample has a stable original_index (for failed-index tracking/filtering)
        # If a sample lacks original_index, use its position in the source data as original_index
        for i, item in enumerate(dataset):
            if isinstance(item, dict) and 'original_index' not in item:
        ...
        if os.path.exists(progress_file):
            with open(progress_file, 'r', encoding='utf-8') as f:
                ...
                start_idx = len(modified_data) + len(failed_indices)
        ...
        for idx in tqdm(range(start_idx, len(dataset)), desc=f"Modifying dataset (mode: {mode})"):
            ...
            if (idx + 1) % save_interval == 0:
                self._save_progress(modified_data, failed_indices)
        
        # Final save
        self._save_final_results(modified_data, failed_indices, f"{dataset_path} (mode: {mode})", mode=mode)
    ...
    def _save_progress(self, modified_data: List[Dict], failed_indices: List[int]):
        """Save progress"""
        progress_file = os.path.join(self.output_dir, "progress.json")
    ...
    def _save_final_results(
        ...
        if mode == "both":
            ...
            self._save_single_mode_results(modified_data, failed_indices, dataset_name)
...
def main():
    ...
    parser.add_argument(
        ...
        default="paraphrased",
        help="Modification mode: 'paraphrased' modifies the paraphrased version, 'original' modifies the original version, 'both' modifies both simultaneously using the same number mapping, 'clean' modifies question-answer-solution format data"
    )
    parser.add_argument(
        "--api-key",
        type=str,
